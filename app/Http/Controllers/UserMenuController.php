...
class UserMenuController extends RoutingController
{
    ...
    public function index(Request $request)
    {
        ...
        $request->validate([
            ...
            'category'       => 'nullable|string|exists:kategori_produk,kode_kategori',
            'sort_by'        => 'nullable|string|in:nama,harga,created_at',
        ...
        $query->when($request->input('category'), function ($q, $category) {
            $q->where('kode_kategori', $category);
        });
        ...
        // Ambil hasil yang sudah dipaginasi
        // withQueryString() PENTING agar link paginasi tetap membawa parameter filter
        $menus = $query->paginate(12)->withQueryString();

        return Inertia::render('Users/MenuPage', [
            'menu'       => $menus,
            'categories' => $categories,
            // KIRIM KEMBALI FILTER KE FRONTEND! Ini adalah kuncinya.
