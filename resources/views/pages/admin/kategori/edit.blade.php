    ...
    <div class="bg-white min-h-screen py-8 px-4 sm:px-6 lg:px-8 pt-20 w-full">
        <div class=" mx-auto">
            ...
            <div class="mb-6">
                ...
                <p class="mt-2 text-gray-600">Update category details for: {{ $category->nama }}</p>
            </div>

            <!-- Form Container -->
            <div class="bg-white shadow overflow-hidden rounded-lg">
                <form action="{{ route('admin.kategori.update', $category->kode_kategori) }}" method="POST"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" value="{{ $category->kode_kategori }}"
                            class="form-input p-2 w-full rounded-md border-gray-300 bg-gray-100 text-gray-700" readonly>
                    </div>

                    <!-- Category Name -->
                    <div class="mb-4">
                        <label for="nama" class="block text-sm font-medium text-gray-700 mb-1">Category Name</label>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="4"
                            class="form-textarea p-2 w-full rounded-md border-gray-300 shadow-sm focus:border-[#6F4E37] focus:ring focus:ring-[#F5E6DD]">{{ old('deskripsi', $category->deskripsi) }}</textarea>
                    </div>

                    <!-- Category Image -->
                    <div class="mb-6">
                        <label for="path_img" class="block text-sm font-medium text-gray-700 mb-1">Category
                            Image</label>
                        <div class="mt-1 flex items-center">
                            <div
                                class="preview-container h-32 w-32 bg-gray-100 rounded flex items-center justify-center mb-2">
                                @if ($category->path_img)
                                    <img id="image-preview" src="{{ asset('storage/' . $category->path_img) }}"
                                        alt="{{ $category->nama }}" class="h-full w-full object-cover rounded">
                                @else
    ...
    <script>
        function previewImage(input) {
            ...
            if (input.files && input.files[0]) {
                ...
                if ('{{ $category->path_img }}') {
                    preview.src = '{{ asset('storage/' . $category->path_img) }}';
                } else {
                    preview.src = '#';
