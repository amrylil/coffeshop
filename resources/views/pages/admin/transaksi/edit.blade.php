...
@section('content')
    <div class="bg-white min-h-screen py-8 px-4 sm:px-6 lg:px-8 pt-20">
        <div class="max-w-3xl mx-auto">
            <h1 class="text-3xl font-bold text-[#6F4E37] mb-6">Edit Transaksi</h1>
                ...
                <div>
                    ...
                    <select name="status_222297" id="status_222297"
                        ...
                        <option value="pending"
                            {{ old('status_222297', $transaksi->status_222297) == 'pending' ? 'selected' : '' }}>Pending
                        </option>
                        <option value="dikonfirmasi"
                            {{ old('status_222297', $transaksi->status_222297) == 'dikonfirmasi' ? 'selected' : '' }}>
                            Dikonfirmasi</option>
                        <option value="ditolak"
                            {{ old('status_222297', $transaksi->status_222297) == 'ditolak' ? 'selected' : '' }}>Ditolak
                        </option>
                        <option value="selesai"
                            {{ old('status_222297', $transaksi->status_222297) == 'selesai' ? 'selected' : '' }}>Selesai
                        </option>
                    </select>
                    @error('status_222297')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
