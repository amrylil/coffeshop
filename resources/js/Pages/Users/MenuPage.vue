        ...
        <div v-else class="max-w-7xl mx-auto px-4">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            <CardMenu
              ...
              @add-to-cart="handleAddToCart(item.kode_menu, item.nama)"
              @click="router.visit(route('menu.show', item.kode_menu))"
              class="fade-in"
            />
          </div>
...
import { useScrollToTop, useCart, useToast, useModal, useMenuFilter } from '@/composables/useMenu';
...
const { cartCount, loadCartCount, addToCart } = useCart();
...
const handleAddToCart = async (menuId: string, menuName: string) => {
  ...
  try {
    ...
    if (result.success) {
      ...
      toast.showToast(result.message || 'Failed to add item to cart', 'error');
    }
  } catch (error) {
